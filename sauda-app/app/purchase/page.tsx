...
type Result = { rowsTotal: number; rowsMatched: number; unmatched: string[] };
    ...
    <main className="max-w-2xl mx-auto p-4 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Upload the dispatch Excel from today&apos;s mail — matched rows update dispatched quantity automatically.
        </p>
      </div>
      ...
      {result && (
        <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
          <p className="text-sm">
            <span className="font-medium">{result.rowsMatched}</span> of {result.rowsTotal} rows matched and applied.
          </p>

          {result.unmatched.length > 0 && (
            <div className="space-y-3">
              <p className="text-sm text-amber-700">
                {result.unmatched.length} product(s) need a one-time alias mapping — pick the matching item below, once. Future uploads with this exact name will apply automatically.
              </p>
              {result.unmatched.map((product) => (
              ...
              <p className="text-xs text-gray-500">
                Note: mapped aliases apply going forward — re-upload this file after saving mappings to apply this batch too.
              </p>
            </div>
          )}
        </div>
